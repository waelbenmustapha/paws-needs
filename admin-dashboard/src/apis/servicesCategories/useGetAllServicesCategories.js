...
const fetch = (page, perPage,search) => {
  return axios.get(
    `http://localhost:3000/servicesCategory/get?search=${search?search:""}&page=${page}&perpage=${perPage}`,
    {
      headers: {
...
export const useGetAllServicesCategories = (page, perPage,search) => {
  return useQuery({
    queryKey: ["fetch-services-categories", page, perPage,search],
