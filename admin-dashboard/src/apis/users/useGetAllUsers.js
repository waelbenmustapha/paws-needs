...
const fetch = (page, perPage,search) => {
  return axios.get(
    `http://localhost:3000/users/get-all-users?search=${search?search:""}&page=${page}&perpage=${perPage}`,
    {
      headers: {
...
export const useGetAllUsers = (page, perPage,search) => {
  return useQuery({
    queryKey: ["fetch-users", page, perPage,search],
