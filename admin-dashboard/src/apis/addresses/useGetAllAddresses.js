...
const fetch = (page, perPage, search) => {
  return axios.get(
    `http://localhost:3000/address/get-all?search=${search?search:""}&page=${page}&perpage=${perPage}`,
    {
      headers: {
...
export const useGetAllAddresses = (page, perPage,search) => {
  return useQuery({
    queryKey: ["fetch-addresses", page, perPage,search],
