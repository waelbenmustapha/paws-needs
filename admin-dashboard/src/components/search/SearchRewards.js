...
function SearchRewards({ keyword, limit, header, setShowSearch }) {
  ...
  const fetchBadges = async () => {
    ...
    const res = await axios.get(
      "https://mocki.io/v1/2ab8f523-1238-4515-837d-494606c45d46"
    );
    setData(res.data);
    filterData(res.data);
  ...
  return (
    <div>
      <div className="flex flex-nowrap justify-between mb-[10px] px-[10px] items-center">
        <p className="font-bold text-[13px]">{header && "Rewards"}</p>
        <span
          onClick={() => {
            navigate("/dashboard/rewards", { state: { data: filtered } });
            setShowSearch(false);
          }}
      ...
      <div className="py-2 px-2 text-center mx-2 text-xs flex gap-[10px] justify-between font-bold whitespace-nowrap">
        ...
        <p className="flex-1 min-w-0 overflow-ellipsis whitespace-nowrap overflow-hidden maxSmallPhone:text-[7px] maxPhone:text-[10px]">Description</p>{" "}
        <p className="flex-1 min-w-0 overflow-ellipsis whitespace-nowrap overflow-hidden maxSmallPhone:text-[7px] maxPhone:text-[10px]">Earned By</p>{" "}
        <p className="flex-1 min-w-0 overflow-ellipsis whitespace-nowrap overflow-hidden maxSmallPhone:text-[7px] maxPhone:text-[10px]">Points</p>{" "}
        <p className="flex-1 min-w-0 overflow-ellipsis whitespace-nowrap overflow-hidden maxSmallPhone:text-[7px] maxPhone:text-[10px]">Date Created</p>{" "}
  ...
  return (
    <div className={style}>
      <div className="py-2 px-2 text-center mx-2 text-xs flex gap-[10px]  justify-between font-bold whitespace-nowrap">
        ...
        <p className="flex-1 overflow-ellipsis whitespace-nowrap overflow-hidden  maxPhone:text-[10px] min-w-0 ">
          {element.description}
        </p>{" "}
        <p className="flex-1 overflow-ellipsis whitespace-nowrap overflow-hidden  maxPhone:text-[10px] min-w-0 ">
          {element.earnedBy}
